...
const translateOracle = (sql) => {
  ...
  s = s.replace(/\bADD_MONTHS\s*\(\s*([^,]+),\s*([^)]+)\)/gi, "date($1, '+' || ($2) || ' months')");
  s = s.replace(/\bTO_CHAR\s*\(([^,)]+)(?:,[^)]+)?\)/gi, "CAST($1 AS TEXT)");
  s = s.replace(/\bTO_DATE\s*\(([^,)]+)(?:,[^)]+)?\)/gi, "$1");
  s = s.replace(/\bTO_NUMBER\s*\(([^)]+)\)/gi, "CAST($1 AS NUMERIC)");
  ...
  s = s.replace(/'(\d{1,2})-([A-Z]{3})-(\d{2,4})'/gi, (match, d, m, y) => {
    const mm = months[m.toUpperCase()] || '01';
...
console.log(translateOracle("INSERT INTO employees VALUES (3,'Carol','White',75000,1,1,'22-JUL-2018');"));